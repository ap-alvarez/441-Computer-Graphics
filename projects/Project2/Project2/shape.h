...
public:
    ...
    virtual float refraction() const {
        return 0;
    }
...
public:
    Sphere(
        ...
        float reflection=0,
        float refraction=0
    ) : Shape(), _center(center), _radius(radius), _color(color), reflect(reflection), refract(refraction) {}
    ...
    glm::vec3 center() const { return _center; }
     ...
     float refraction() const override {
         return refract;
     }
...
struct Obj {
    ...
    static std::vector<Triangle> make_box(const glm::vec3 c, float s, const glm::vec3 color, bool reflective=false) {
        return {
            Triangle(c+s*glm::vec3(-0.5f, -0.5f, -0.5f), c+s*glm::vec3( 0.5f, -0.5f, -0.5f), c+s*glm::vec3( 0.5f,  0.5f, -0.5f), color, reflective),
            Triangle(c+s*glm::vec3( 0.5f,  0.5f, -0.5f), c+s*glm::vec3(-0.5f,  0.5f, -0.5f), c+s*glm::vec3(-0.5f, -0.5f, -0.5f), color, reflective),
