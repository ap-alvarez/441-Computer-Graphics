...
void add_vertex(T& coords, const Vertex& v) {
    ...
    coords.push_back(v.nx);
    coords.push_back(v.ny);
    coords.push_back(v.nz);
}
...
class Sphere : public Shape {
    ...
    Vertex vertex(float radius, float phi, float theta) {
        ...
        Vector4 norm = Vector4(vert.x, vert.y, vert.z).normalized();
        vert.nx = double(norm.x());
        vert.ny = double(norm.y());
        vert.nz = double(norm.z());
        return vert;
    }
